...
const BlocklyLoading = observer(() => {
    ...
    return is_loading ? (
        <div className='bot__loading derivfx-loader' data-testid='blockly-loader' style={{ background: 'radial-gradient(circle at 50% 35%, #102a56 0%, #071426 42%, #020711 100%)', color: '#fff', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: 7, alignItems: 'center', height: 30 }}>
                {[0, 1, 2].map(index => (
                    <span key={index} style={{ width: 8, height: 8, borderRadius: '50%', background: index === 1 ? '#ff3b43' : '#16c8ff', animation: `derivfx-pulse 1s ease-in-out ${index * 0.16}s infinite` }} />
                ))}
            </div>
